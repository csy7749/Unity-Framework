...
namespace UnityFramework
{
    ...
    public class GuideGesture : GuideWidgetBase
    {
        ...
        public override void Init(GuideWidgetData data)
        {
            ...
            if (gestureData != null)
            {
                ...
                if (gestureData.Open)
                {
                    ...
                    if (gestureData.gestureType == GestureType.ThumbDrag ||
                        gestureData.gestureType == GestureType.ForeFingerDrag)
                    {
        ...
        public GameObject LoadGesture(GestureType type)
        {
            ...
            {
                GestureAnimation =
                    Instantiate(ResourceManager.Load<GameObject>("Gesture/clickPrefab_thumb"), transform);
            }
        ...
        private IEnumerator DOLocalMove(Vector3 endValue, float duration, AnimationCurve animationCurve)
        {
            ...
            while (Time.time - startTime <= duration)
            {
                transform.localPosition = startPos +
                                          (endValue - startPos) *
                                          animationCurve.Evaluate((Time.time - startTime) / duration);
                yield return null;
            }
        }
        ...
        public void StopAnimation()
        {
            ...
            //只有拖动手势需要关闭额外的位移动画
            if ((gestureData.gestureType == GestureType.ThumbDrag ||
                 gestureData.gestureType == GestureType.ForeFingerDrag) && dragTween != null)
            {
