...
namespace UnityFramework.Editor.UXTools
{
    public class PrefabLabelModifyWindow : EditorWindow
    {
        ...
        public static void OpenWindow(string label, Action<string> action = null)
        {
            ...
            m_window.titleContent.text = "组件类型";
        ...
        private void Submit()
        {
            if (EditorUtility.DisplayDialog("messageBox", "是否要保存修改", "确定", "取消"))
            {
                if (!string.IsNullOrEmpty(currentName))
                {
                    ...
                    {
                        EditorUtility.DisplayDialog("messageBox", "组件类型重复", "确定", "取消");
                        return;
                    }
                }
                else
                {
                    EditorUtility.DisplayDialog("messageBox", "请输入组件类型名称", "确定", "取消");
                    return;
                }
