    ...
    public class SuperCharacterController : MonoBehaviour
    {
        ...
        bool SlopeLimit()
        {
            ...
            if (a > currentGround.superCollisionType.SlopeLimit)
            {
                ...
                if (Physics.CapsuleCast(SpherePosition(feet), SpherePosition(head), radius, direction.normalized,
                        out hit, direction.magnitude, Walkable, triggerInteraction))
                {
            ...
            foreach (var sphere in spheres)
            {
                             ...
                             triggerInteraction))
                {
                    ...
                    bool contactPointSuccess =
                        SuperCollider.ClosestPointOnSurface(col, position, radius, out contactPoint);
                    ...
                    if (v != Vector3.zero)
                    {
                        ...
                        bool facingNormal = Physics.SphereCast(new Ray(position, v.normalized), TinyTolerance,
        ...
        public class SuperGround
        {
            ...
            public void ProbeGround(Vector3 origin, int iter)
            {
                ...
                if (Physics.SphereCast(o, smallerRadius, down, out hit, Mathf.Infinity, walkable, triggerInteraction))
                {
                    ...
                    Vector3 toCenter = Math3d.ProjectVectorOnPlane(up,
                ...
                else if (Physics.Raycast(o, down, out hit, Mathf.Infinity, walkable, triggerInteraction))
                {
                    ...
                    {
                        primaryGround = new GroundHit(sphereCastHit.point, sphereCastHit.normal,
                            sphereCastHit.distance);
                    }
                    else
                    {
            ...
            private bool OnSteadyGround(Vector3 normal, Vector3 point)
            {
                ...
                float distanceRatio =
                    Mathf.Lerp(groundingMinPercentFromcenter, groundingMaxPercentFromCenter, angleRatio);
