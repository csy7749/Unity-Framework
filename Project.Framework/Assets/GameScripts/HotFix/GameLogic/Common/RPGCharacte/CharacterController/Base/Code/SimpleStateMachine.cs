    ...
    public class SimpleStateMachine : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            if (DebugGui)
            {
                ...
                GUI.TextField(new Rect(DebugGuiPosition.x + 10, DebugGuiPosition.y + 20, 180, 20),
                    string.Format("State: {0}", currentState));
            }
        }
        ...
        T ConfigureDelegate<T>(string methodRoot, T Default) where T : class
        {
            ...
            {
                var mtd = GetType().GetMethod(state.currentState.ToString() + "_" + methodRoot,
