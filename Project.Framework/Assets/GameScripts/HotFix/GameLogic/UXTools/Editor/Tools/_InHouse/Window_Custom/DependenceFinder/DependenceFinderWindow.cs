...
namespace ThunderFireUITool
{
    public class DependenceFinderWindow : EditorWindow
    {
        ...
        private static void FindDepend()
        {
            ...
            _mWindow.titleContent.text = "查找依赖资源";
        ...
        private void RefreshLeftTypeList()
        {
            ...
            // 添加左侧分类按钮
            CreateBtn(ResourceType.Default, "全部");
            CreateBtn(ResourceType.Picture, "图片类型");
            CreateBtn(ResourceType.Prefab, "Prefab类型");
            CreateBtn(ResourceType.Anim, "动画类型");
            CreateBtn(ResourceType.Material, "材质类型");
            return;
