...
namespace UnityEngine.UI
{
    ...
    public class UXImageEditor : GraphicEditor
    {
        ...
        protected override void OnEnable()
        {
            ...
            m_FlipModeContent = new GUIContent("镜像模式");
            m_FlipEdgeContent = new GUIContent("镜像方向");
            m_FlipFillContent = new GUIContent("填充中心点");
            ...
            m_CustomList.drawHeaderCallback = (Rect rect) =>
            {
                ...
                if (validNums != UXGUIConfig.availableLanguages.Count)
                {
                    ...
                    GUI.Label(GUILayoutUtility.GetLastRect(), "请检查文件夹中图片的命名规范！");
                }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            m_IgnoreLocalization.boolValue = !EditorGUILayout.Toggle("开启本地化", !m_IgnoreLocalization.boolValue);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            if (!m_IgnoreLocalization.boolValue)
            {
                ...
                if (m_Sprite.objectReferenceValue != null && GUILayout.Button(new GUIContent("导入所有语种图片",
                "可同时导入多张图片，文件命名：图片名_语言缩写，例：battlemessage_CN ")))
                {
            ...
            {
                EditorGUILayout.HelpBox("当前 RectTransform 的宽度或高度为 0，这样可能会导致渲染问题或功能异常。请确保在使用镜像模式时，RectTransform 的宽度和高度都大于 0。", MessageType.Warning);
            }
        ...
        {
            EditorGUI.BeginChangeCheck();
            string[] labels = { "纯色", "渐变色" };
            var type = (UXImage.ColorType)Utils.EnumPopupLayoutEx("颜色类型", typeof(UXImage.ColorType), m_ColorType.intValue, labels);
            // EditorGUILayout.EnumPopup("颜色类型", (UXImage.ColorType)m_ColorType.intValue);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            if (GUILayout.Button("使用UIColorConfig", EditorStyles.miniButton))
            {
                //弹窗
            ...
            {
                EditorGUI.BeginChangeCheck();
                string[] labels2 = { "垂直", "水平" };
                var direction = (UXImage.GradientDirection)Utils.EnumPopupLayoutEx("渐变方向", typeof(UXImage.GradientDirection), m_Direction.intValue, labels2);
                if (EditorGUI.EndChangeCheck())
                {
                    m_Direction.intValue = (int)direction;
                }
                //EditorGUILayout.ColorField(m_Color);
                EditorGUILayout.PropertyField(m_GradientColor, new GUIContent("渐变色"));
            }
        ...
        private void FlipGUI()
        {
            ...
            switch (flipmodeEnum)
            {
                case UXImage.FlipMode.Horziontal:
                    ...
                    m_FlipWithCopy.boolValue = EditorGUILayout.Toggle("复制", m_FlipWithCopy.boolValue);
        ...
        private void MaterialEffectGUI()
        {
            ...
            EditorGUI.indentLevel++;
            EditorGUI.BeginChangeCheck();
            m_GammaToLinear.boolValue = EditorGUILayout.Toggle("线性空间", m_GammaToLinear.boolValue);
            if (EditorGUI.EndChangeCheck())
            {
                serializedObject.ApplyModifiedProperties();
            ...
            EditorGUI.BeginChangeCheck();
            m_LinearToGamma.boolValue = EditorGUILayout.Toggle("Gamma空间", m_LinearToGamma.boolValue);
            ...
            {
                EditorGUI.indentLevel++;
                m_Contrast.floatValue = EditorGUILayout.Slider("对比度", m_Contrast.floatValue, 0f, 2.0f);
                m_Saturation.floatValue = EditorGUILayout.Slider("饱和度", m_Saturation.floatValue, 0f, 2.0f);
                EditorGUI.indentLevel--;
            }
            if (EditorGUI.EndChangeCheck())
            {
