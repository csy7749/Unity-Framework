...
namespace GameLogic.Binding.Proxy.Targets.UGUI
{
    public class UnityTargetProxyFactory : ITargetProxyFactory
    {
        ...
        protected virtual ITargetProxy CreateUnityPropertyProxy(object target, IProxyPropertyInfo propertyInfo,
            UnityEventBase updateTrigger)
        {
            ...
            switch (typeCode)
            {
                ...
                default:
                {
                    ...
                    if (valueType.Equals(typeof(Vector2)))
                        return new UnityPropertyProxy<Vector2>(target, propertyInfo,
                            (UnityEvent<Vector2>)updateTrigger);
                    else if (valueType.Equals(typeof(Vector3)))
                        return new UnityPropertyProxy<Vector3>(target, propertyInfo,
                            (UnityEvent<Vector3>)updateTrigger);
                    else if (valueType.Equals(typeof(Vector4)))
                        return new UnityPropertyProxy<Vector4>(target, propertyInfo,
                            (UnityEvent<Vector4>)updateTrigger);
                    else
                        return (ITargetProxy)Activator.CreateInstance(
                            typeof(UnityPropertyProxy<>).MakeGenericType(valueType), target, propertyInfo,
                            updateTrigger); //JIT Exception
                }
            }
        }

        protected virtual ITargetProxy CreateUnityFieldProxy(object target, IProxyFieldInfo fieldInfo,
            UnityEventBase updateTrigger)
        {
            ...
            switch (typeCode)
            {
                ...
                default:
                {
                    ...
                    if (valueType.Equals(typeof(Vector2)))
                        ...
                        return (ITargetProxy)Activator.CreateInstance(
                            typeof(UnityFieldProxy<>).MakeGenericType(valueType), target, fieldInfo,
                            updateTrigger); //JIT Exception
                }
            }
        }

        protected virtual ITargetProxy CreateUnityEventProxy(object target, UnityEventBase unityEvent,
            Type[] paramTypes)
        {
                    ...
                    switch (typeCode)
                    {
                        ...
                        default:
                        {
                            ...
                            if (valueType.Equals(typeof(Vector2)))
                                ...
                                return (ITargetProxy)Activator.CreateInstance(
                                    typeof(UnityEventProxy<>).MakeGenericType(valueType), target,
                                    unityEvent); //JIT Exception
                        }
                    }
                case 2:
                    return (ITargetProxy)Activator.CreateInstance(
                        typeof(UnityEventProxy<,>).MakeGenericType(paramTypes), target,
                        unityEvent); //If creating an exception, define a static type:static Type t = tyeof(UnityEventProxy<P1,P2>)
                case 3:
                    return (ITargetProxy)Activator.CreateInstance(
                        typeof(UnityEventProxy<,,>).MakeGenericType(paramTypes), target, unityEvent);
                case 4:
                    return (ITargetProxy)Activator.CreateInstance(
                        typeof(UnityEventProxy<,,,>).MakeGenericType(paramTypes), target, unityEvent);
                default:
