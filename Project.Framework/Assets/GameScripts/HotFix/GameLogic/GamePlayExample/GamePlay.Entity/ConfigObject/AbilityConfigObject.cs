...
{
    [CreateAssetMenu(fileName = "能力配置", menuName = "Ability/Config")]
    [LabelText("技能配置")]
...
#endif
    {
        ...
        private void AddEffect()
        {
            ...
            {
                var effectType = typeof(Effect).Assembly
                    .GetTypes()
                    .Where(x => !x.IsAbstract)
                    .Where(x => typeof(Effect).IsAssignableFrom(x))
                    .Where(x => x.GetCustomAttribute<EffectAttribute>() != null)
                    .FirstOrDefault(x => x.GetCustomAttribute<EffectAttribute>().EffectType == EffectTypeName);
