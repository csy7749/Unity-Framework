...
public class UIColorConfigWindow : EditorWindow
{
    ...
    {
        var window = GetWindow<UIColorConfigWindow>(true, "颜色预设编辑器", true);
    ...
    private bool NameValid(List<string> colorNames)
    {
        ...
        {
            info += $"{"如下颜色名非法（只能包含字母、数字、下划线）"}\n{invalidNames}\n";
        }
        if (!string.IsNullOrEmpty(sameNames))
        {
            info += $"{"如下颜色名重复"}\n{sameNames}\n";
        }
        EditorUtility.DisplayDialog("Warning", $"{info}", "确定");
        return false;
    }
