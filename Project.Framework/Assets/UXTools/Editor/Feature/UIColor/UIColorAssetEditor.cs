        ...
        reorderableList.drawElementCallback = (rect, index, isActive, isFocused) =>
        {
            ...
            if (index == activeIndex)
            {
                ...
                EditorGUI.PropertyField(
                    ...
                    element.FindPropertyRelative("ColorDefName"), new GUIContent("颜色名"));
                GUI.backgroundColor = Color.white; // 必须要加，否则会影响后续元素的背景颜色
                EditorGUI.PropertyField(
                    new Rect(rect.x, rect.y + 20, rect.width, 20),
                    element.FindPropertyRelative("colorValue"), new GUIContent("颜色"));
                EditorGUI.PropertyField(
                    new Rect(rect.x, rect.y + 40, rect.width, 20),
                    element.FindPropertyRelative("ColorComment"), new GUIContent("颜色备注"));
            }
            else
            {
                EditorGUI.PropertyField(
