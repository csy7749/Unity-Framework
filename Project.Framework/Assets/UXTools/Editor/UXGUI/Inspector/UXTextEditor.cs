...
namespace UnityEngine.UI
{
    ...
    public class UXTextEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            var type = Utils.EnumPopupLayoutEx("文本类型",
            ...
            {
                EditorGUI.BeginChangeCheck();
                ignoreLocalization.boolValue = !EditorGUILayout.Toggle("开启本地化", !ignoreLocalization.boolValue);
                if (EditorGUI.EndChangeCheck())
                {
                    ShowObj();
                ...
                if (!ignoreLocalization.boolValue)
                {
                    ...
                    foldout = EditorGUILayout.BeginFoldoutHeaderGroup(foldout, "表格使用提示");
                    if (foldout)
                    {
                        GUI.enabled = false;
                        GUIStyle style = new GUIStyle(EditorStyles.textArea);
                        style.padding = new RectOffset(5, 5, 5, 5);
                        GUILayout.TextArea("提示：更新表格并保存后，点击菜单栏-ThunderFireUXTool-本地化-将文本表格转为JSON文件，才能正常预览和切换语言。 ", style);
                        GUI.enabled = true;
                    }
                    ...
                    GUI.enabled = true;
                    if (GUILayout.Button("复制", GUILayout.MaxWidth(50)))
                    {
            ...
            else if (type == (int)LocalizationHelper.TextLocalizationType.Preview)
            {
                ...
                {
                    EnableArabicFix.boolValue = EditorGUILayout.Toggle("使用阿拉伯语修正", EnableArabicFix.boolValue);
                }
            ...
            EditorGUI.BeginChangeCheck();
            var ellipsistype = Utils.EnumPopupLayoutEx("省略号类型",
        ...
        private void DrawStyle(UXText targetObject)
        {
            ...
            if (targetObject.resizeTextForBestFit && targetObject.horizontalOverflow == HorizontalWrapMode.Wrap)
            {
                ...
                bool useShrinkFit = EditorGUILayout.Toggle("自适应文本框", m_useShrinkFit.boolValue);
                --EditorGUI.indentLevel;
                if (EditorGUI.EndChangeCheck())
                {
