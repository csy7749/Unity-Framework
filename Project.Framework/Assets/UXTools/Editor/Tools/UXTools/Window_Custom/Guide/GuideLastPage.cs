...
public class GuideLastPage : VisualElement
{
    public GuideLastPage(VisualElement veParent)
    {
        ...
        UXBuilder.Text(content, new UXBuilderTextStruct()
        {
            ...
            text = "请根据需求设置功能开关\n（之后可随时在ThunderFireUXTool-设置-功能开关中修改设置）"
        });
    ...
    private void DrawRowToggles(VisualElement veParent)
    {
        ...
        AddToggle(colLeft, "对齐吸附", 1);
        AddToggle(colLeft, "右键选择列表", 2);
        AddToggle(colLeft, "快速复制", 3);
        AddToggle(colLeft, "移动快捷键", 4);
        ...
        AddToggle(colRight, "Prefab多开", 5);
        AddToggle(colRight, "Scene中分辨率调整", 6);
        AddToggle(colRight, "Prefab资源检查", 7);
        ...
        AddToggle(colRight, "自动将Texture转为Sprite格式", 8);
    }
